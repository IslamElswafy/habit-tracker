// هذا سكريبت لإضافة العادات تلقائياً إلى Firebase
// تشغيله: node scripts/add-habits.js
...
// معرف الجهاز - يمكن تغييره
const deviceId = "device_main_" + Date.now();
...
async function addHabits() {
  ...
  for (const habit of habits) {
    try {
      ...
      console.log(
        `✅ تمت إضافة: ${habit.title} (${habit.category}) - ${habit.points} نقطة`
      );
      successCount++;
    } catch (error) {
  ...
  console.log("\n⚠️ مهم: احفظ معرف الجهاز هذا للوصول للعادات من أي مكان!");
